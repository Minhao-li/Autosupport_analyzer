...
import { fmtBytes, FilterInput, matchKeywords, SearchHelp, ExpandToggle } from "../lib/helpers.jsx";
import EventsTable from "./EventsTable.jsx";
import GrepModal from "./GrepModal.jsx";
import InlineContent from "./InlineContent.jsx";

export default function SnapshotView({ caseId }) {
  ...
  useEffect(() => {
    ...
    api.snapshot(caseId).then((r) => setFiles(r.files || [])).catch((e) => setErr(String(e.message || e)));
  }, [caseId]);
  ...
  // Group files by their autosupport collection directory (the segment after
  // ".../autosupport/"), or otherwise by their parent directory.
  const groupKey = (path) => {
  ...
  const parse = async (which) => {
    const paths = which === "all" ? shown.map((f) => f.path) : [...sel];
    if (!paths.length) return;
    setBusy(true); setErr(null);
    try {
      const r = await api.snapshotEvents(caseId, { files: paths.join(",") });
  ...
  return (
    <div style={{ paddingBottom: grep ? "47vh" : undefined }}>
      <h2 className="content-title">Snapshot</h2>
      <p className="content-subtitle">{(files || []).length} file(s) in this case</p>
      ...
      {tab === "files" && (
        <>
          <div className="toolbar">
            ...
            <SearchHelp />
            <button className="btn" onClick={() => parse("all")} disabled={busy}>Parse all</button>
            <button className="btn" onClick={() => parse("sel")} disabled={busy || !sel.size}>Parse selected ({sel.size})</button>
            <button className="btn" disabled={!sel.size} onClick={() => setGrep([...sel])}>Grep selected ({sel.size})</button>
            <button className="btn" onClick={() => setGrep((files || []).map((f) => f.path))}>Grep all</button>
          </div>

          {!files ? <div className="info-text"><span className="spin" /> Loading…</div> : shown.length === 0 ? (
            <div className="empty-state">No files in this case.</div>
          ) : (
            <div className="split">
      ...
      {grep && <GrepModal caseId={caseId} paths={grep}
        onOpenFile={(p) => { setActive(p); setTab("files"); }}
